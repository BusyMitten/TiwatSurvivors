// HopeCraft - A block game written in C++
// Author: BusySheng
// Date: 2025/10/1
#include<graphics.h>
#include<string>
...
public:
	...
	const POINT& GetPosition() const { return player_pos; }
private:
    const int SPEED = 3;
...
public:
    ...
    bool CheckPlayerCollision(const Player& player)
    {
		...
		bool is_overlap_x = position.x + FRAME_WIDTH / 2 >= player_position.x
            && position.x + FRAME_WIDTH / 2 <= player_position.x + PLAYER_WIDTH;
        bool is_overlap_y = position.y + FRAME_HEIGHT / 2 >= player_position.y 
            && position.y + FRAME_HEIGHT / 2 <= player_position.y + PLAYER_HEIGHT;
		return is_overlap_x && is_overlap_y;
    }
    void Move(const Player& player)
    {
        ...
        if (len_dir != 0)
        {
            ...
            if((int)(normalized_x * SPEED)>0)
                facing_left = false;
            else if((int)(normalized_x * SPEED)<0)
				facing_left = true;
		}
    }
    void Draw(int delta)
    {
		...
		bar3d(position.x + (FRAME_WIDTH - hp*3) / 2, position.y - 10,
			position.x + (FRAME_WIDTH + hp*3) / 2, position.y - 2, 1, true);
    }
...
void UpdateBullets(std::vector<Bullet>& bullet_list, const Player& player)
{
	...
	double randian_interbal = 2 * 3.1415926 / bullet_list.size();
	POINT player_position = player.GetPosition();
	double radius = 100 + 25 * sin(GetTickCount() * RADIAL_SPEED);
    for (size_t i = 0; i < bullet_list.size(); i++)
    {
		double radian = GetTickCount() * TANGENT_SPEED + i * randian_interbal;
		bullet_list[i].position.x = player_position.x + PLAYER_WIDTH / 2 + (int)(radius * sin(radian));
		bullet_list[i].position.y = player_position.y + PLAYER_HEIGHT / 2 + (int)(radius * cos(radian));
    }
}
...
int APIENTRY WinMain(_In_ HINSTANCE hInstance,_In_opt_ HINSTANCE hPrevInstance,_In_ LPSTR lpCmdLine,_In_ int nCmdShow)
{
    ...
    while (running)
    {
        ...
        if (delta_time == 0) delta_time = 1; // 防止除零
        last_time = now_time;
